...
function helloBtnFirstColor() {
  sayHelloButton.style.backgroundColor = '#EFEFEF';
  sayHelloButton.style.color = 'black';
}

sayHelloButton.addEventListener('mouseout', helloBtnFirstColor);
...
const repeatMyParam = () => {
  axios.get('http://localhost:3000/repeat/{Whatsupppp!!!}').then((response) => {
    console.log(response.data);
    document.getElementById('repeat-text').textContent = response.data.value;
  });
};
...
const queryFunc = () => {
  axios
    .get('http://localhost:3000/query-test?what is your favorite color')
    .then((res) => {
      console.log(res.data);
    });
};
...
/*
    ...
    form and write a function that makes a post request. Then you'll attach that function to the submit event 
    on the form. We'll be creating a list of foods. 
